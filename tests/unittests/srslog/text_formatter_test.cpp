...
static detail::log_entry_metadata build_log_entry_metadata(fmt::dynamic_format_arg_store<fmt::format_context>* store)
{
  ...
  uint64_t ctx_value = ((uint64_t(99) << 32) | uint64_t(99));
  return {tp, {ctx_value, true}, "Text {}", store, "ABC", 'Z'};
}
...
DECLARE_METRIC_SET("sector_metrics", sector_set, entry_type_t, sector_id_t, valarray_t, valopt_t, ue_list_t);
...
static complex_ctx_t build_complex_context()
{
  ...
  ctx.at<sector_list_t>(0).write<valopt_t>(std::nullopt);
