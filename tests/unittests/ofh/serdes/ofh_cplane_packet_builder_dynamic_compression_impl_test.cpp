...
TEST(ofh_control_plane_packet_builder_dynamic_compression_impl_test, uplink_packet_codifies_ud_comp_header_bfp9)
{
  std::vector<uint8_t> packet = {
  ...
  ASSERT_EQ(packet, result_packet);
  ASSERT_EQ(nof_bytes, packet.size());
  // Check that the udCompHdr is serialised.
...
TEST(ofh_control_plane_packet_builder_dynamic_compression_impl_test, uplink_packet_codifies_ud_comp_header_nocompr16)
{
  std::vector<uint8_t> packet = {
  ...
  ASSERT_EQ(nof_bytes, packet.size());
  // Check that the udCompHdr is serialised.
  // 0 is used to encode a bitwidth of 16 bits.
  ASSERT_EQ(0, result_packet[UD_COMP_HEADER_BYTE] >> 4);
  ASSERT_EQ(packet_params.compr_params.type, to_compression_type(result_packet[UD_COMP_HEADER_BYTE] & 0xf));
}
