...
private:
    ...
    HttpContext<SSL> *init() {
        ...
        us_socket_context_on_data(SSL, getSocketContext(), [](us_socket_t *s, char *data, int length) {
            ...
            auto [err, returnedSocket] = httpResponseData->consumePostPadded(data, (unsigned int) length, s, proxyParser, [httpContextData](void *s_, HttpRequest *httpRequest) -> void * {
                /* For every request we reset the timeout and hang until user makes action */
                /* Warning: if we are in shutdown state, resetting the timer is a security issue! */
                us_socket_timeout(SSL, (us_socket_t *) s_, 0);

                /* Reset httpResponse */
                HttpResponseData<SSL> *httpResponseData_ = (HttpResponseData<SSL> *) us_socket_ext(SSL, (us_socket_t *) s_);
                httpResponseData_->offset = 0;
                ...
                if (httpResponseData_->state & HttpResponseData<SSL>::HTTP_RESPONSE_PENDING) {
                    us_socket_close(SSL, (us_socket_t *) s_, 0, nullptr);
                    return nullptr;
                }

                /* Mark pending request and emit it */
                httpResponseData_->state = HttpResponseData<SSL>::HTTP_RESPONSE_PENDING;
                ...
                if (httpRequest->isAncient() || httpRequest->getHeader("connection").length() == 5) {
                    httpResponseData_->state |= HttpResponseData<SSL>::HTTP_CONNECTION_CLOSE;
                }

                /* Select the router based on SNI (only possible for SSL) */
                auto *selectedRouter = &httpContextData->router;
                if constexpr (SSL) {
                    void *domainRouter = us_socket_server_name_userdata(SSL, (struct us_socket_t *) s_);
                    if (domainRouter) {
                ...
                selectedRouter->getUserData() = {(HttpResponse<SSL> *) s_, httpRequest};
                ...
                if (us_socket_is_closed(SSL, (struct us_socket_t *) s_)) {
                    return nullptr;
                }

                /* We absolutely have to terminate parsing if shutdown */
                if (us_socket_is_shut_down(SSL, (us_socket_t *) s_)) {
                    return nullptr;
                }

                /* Returning from a request handler without responding or attaching an onAborted handler is ill-use */
                if (!((HttpResponse<SSL> *) s_)->hasResponded() && !httpResponseData_->onAborted) {
                ...
                if (!((HttpResponse<SSL> *) s_)->hasResponded() && httpResponseData_->inStream) {
                    us_socket_timeout(SSL, (us_socket_t *) s_, HTTP_IDLE_TIMEOUT_S);
                }

                /* Continue parsing */
                return s_;

            }, [httpResponseData](void *user, std::string_view data_, bool fin) -> void * {
                ...
                if (httpResponseData->inStream) {
                    ...
                    if (fin) {
                        ...
                        httpResponseData->received_bytes_per_timeout += (unsigned int) data_.length();
                    ...
                    httpResponseData->inStream(data_, fin);
