...
public:
    ...
    TemplatedApp &&missingServerName(MoveOnlyFunction<void(const char *hostname)> handler) {
        ...
        if (!constructorFailed()) {
            ...
            us_socket_context_on_server_name(SSL, (struct us_socket_context_t *) httpContext, [](struct us_socket_context_t *context, const char *hostname) {
                ...
                HttpContext<SSL> *httpContext_ = (HttpContext<SSL> *) context;
                httpContext_->getSocketContextData()->missingServerNameHandler(hostname);
            });
        }
    ...
    bool publish(std::string_view topic, std::string_view message_, OpCode opCode, bool compress = false) {
        /* Anything big bypasses corking efforts */
        if (message_.length() >= LoopData::CORK_BUFFER_SIZE) {
            return topicTree->publishBig(nullptr, topic, {message_, opCode, compress}, [](Subscriber *s, TopicTreeBigMessage &message) {
            ...
            return topicTree->publish(nullptr, topic, {std::string(message_), opCode, compress});
        }
    }
    ...
    TemplatedApp &&ws(std::string pattern, WebSocketBehavior<UserData> &&behavior) {
        ...
        if (!topicTree) {
            ...
            Loop::get()->addPostHandler(topicTree, [topicTree_ = topicTree](Loop */*loop*/) {
                /* Commit pub/sub batches every loop iteration */
                topicTree_->drain();
            });

            Loop::get()->addPreHandler(topicTree, [topicTree_ = topicTree](Loop */*loop*/) {
                /* Commit pub/sub batches every loop iteration */
                topicTree_->drain();
            });
        }
    ...
    TemplatedApp &&addChildApp(TemplatedApp *app) {
        ...
        httpContext->onPreOpen([](struct us_socket_context_t *context, LIBUS_SOCKET_DESCRIPTOR fd) -> LIBUS_SOCKET_DESCRIPTOR {
            ...
            if (httpContext_->getSocketContextData()->childApps.empty()) {
                return fd;
            }

            //std::cout << "Distributing fd: " << fd << " from context: " << context << std::endl;

            unsigned int *roundRobin = &httpContext_->getSocketContextData()->roundRobin;

            //std::cout << "Round robin is: " << *roundRobin << " and size of apps is: " << httpContext->getSocketContextData()->childApps.size() << std::endl;

            TemplatedApp *receivingApp = (TemplatedApp *) httpContext_->getSocketContextData()->childApps[*roundRobin];
            ...
            if (++(*roundRobin) == httpContext_->getSocketContextData()->childApps.size()) {
                *roundRobin = 0;
            }
