...
public:
    ...
    SendStatus send(std::string_view message, OpCode opCode = OpCode::BINARY, bool compress = false, bool fin = true) {
        ...
        if (message.length() >= 16 * 1024 && !compress && !SSL && !webSocketData->subscriber && getBufferedAmount() == 0 && Super::getLoopData()->corkOffset == 0) {
            ...
            if (compress) {
                ...
                if (message.length() && opCode < 3 && webSocketData_->compressionStatus == WebSocketData::ENABLED) {
                    LoopData *loopData = Super::getLoopData();
                    /* Compress using either shared or dedicated deflationStream */
                    if (webSocketData_->deflationStream) {
                        message = webSocketData_->deflationStream->deflate(loopData->zlibContext, message, false);
                    } else {
        ...
        if (webSocketContextData->resetIdleTimeoutOnSend) {
            ...
            WebSocketData *webSocketData_ = (WebSocketData *) Super::getAsyncSocketData();
            webSocketData_->hasTimedOut = false;
        }
    ...
    bool publish(std::string_view topic, std::string_view message, OpCode opCode = OpCode::TEXT, bool compress = false) {
        ...
        if (message.length() >= LoopData::CORK_BUFFER_SIZE) {
            return webSocketContextData->topicTree->publishBig(webSocketData->subscriber, topic, {message, opCode, compress}, [](Subscriber *s, TopicTreeBigMessage &message_) {
                auto *ws = (WebSocket<SSL, true, int> *) s->user;

                ws->send(message_.message, (OpCode)message_.opCode, message_.compress);
            });
        } else {
