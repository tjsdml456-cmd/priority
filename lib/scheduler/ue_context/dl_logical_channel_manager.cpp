...
{
  auto& ch = channels[lcid];
  // DSCP runtime override owns tb_gbr/mbr while air cap is active; do not clobber from config each slot.
  if (ch.air_rate_cap) {
    return;
  }
  ...
  ch.tb_mbr_bps = lc_cfg->qos->runtime_gbr_qos_info->max_br_dl;
}
