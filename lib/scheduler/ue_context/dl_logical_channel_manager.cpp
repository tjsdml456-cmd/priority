...
{
  if (qos.runtime_gbr_qos_info.has_value() or qos.gbr_qos_info.has_value()) {
    if (qos.runtime_qos.average_window_ms.has_value()) {
      return qos.runtime_qos.average_window_ms.value();
    }
...
static void sync_lc_mbr_bps(uint64_t& mbr_bps_out, const logical_channel_config::qos_info& qos)
{
  ...
  if (gbr != nullptr) {
    mbr_bps_out = gbr->max_br_dl;
    if (mbr_bps_out == 0) {
      mbr_bps_out = gbr->gbr_dl;
    }
    return;
  }
  mbr_bps_out = 0;
...
void dl_logical_channel_manager::apply_lc_rate_avg_window(lcid_t lcid)
{
  ...
  // DSCP runtime overlay: only track/MFBR-police when runtime GBR is active (non-GBR DSCP clears it).
  if (not lc_cfg->qos->runtime_gbr_qos_info.has_value()) {
    ch.track_delivered_rate = false;
    ch.mbr_bps              = 0;
    ch.delivered_rate.reset();
    ch.avg_bytes_per_slot.resize(0);
  ...
  if (not get_qos_rate_avg_window_msec(lc_cfg->qos.value()).has_value()) {
    ch.track_delivered_rate = false;
    ch.mbr_bps              = 0;
    ch.delivered_rate.reset();
    ch.avg_bytes_per_slot.resize(0);
  ...
  sync_lc_mbr_bps(ch.mbr_bps, lc_cfg->qos.value());
  ...
  for (logical_channel_config_ptr ch_cfg : *channel_configs) {
    ...
    if (not is_srb(ch_cfg->lcid) and ch_cfg->qos.has_value()) {
      if (const auto win_size_msec = get_qos_rate_avg_window_msec(ch_cfg->qos.value())) {
        sync_lc_mbr_bps(ch.mbr_bps, ch_cfg->qos.value());
        const unsigned win_size_slots = std::max(1U, *win_size_msec * slots_per_sec / 1000);
        ch.avg_bytes_per_slot.resize(win_size_slots);
        ch.track_delivered_rate = true;
        ch.delivered_rate.set_window(std::chrono::milliseconds(*win_size_msec));
