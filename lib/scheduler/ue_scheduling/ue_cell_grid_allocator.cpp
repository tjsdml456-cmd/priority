...
{
  // Cap newTx TB sizing by GBR token bucket so PDSCH TBS matches enforceable MAC bytes (Throughput calc).
  ...
  if (grant_bytes < request.pending_bytes) {
    logger.info("[TOKEN-GRANT] ue={} pending={} budget={} air_cap={} grant={}",
                fmt::underlying(request.user.ue_index()),
                request.pending_bytes,
                token_budget,
                request.user.dl_air_rate_cap_enabled() ? request.user.dl_gbr_air_cap_grant_bytes()
  ...
  auto sched_ctxt = sched_helper::get_newtx_dl_sched_context(
      request.user, cell_alloc[0].slot, request.pdsch_slot, request.interleaving_enabled, grant_bytes);
  ...
  // Setup a DL grant.
  auto result = setup_dl_grant_builder(request.user, sched_ctxt.value(), std::nullopt, grant_bytes);
  ...
  if (is_retx) {
    ...
    if (grant.user->dl_air_rate_cap_enabled() and tbs_cap > 0) {
      ...
      if (mcs_tbs_info.tbs > tbs_cap) {
        // Binary search found no (RB,MCS) under cap — shrink from max until TBS fits.
