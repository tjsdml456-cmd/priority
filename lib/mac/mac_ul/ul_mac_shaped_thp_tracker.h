...
/// Fixed-window UL MAC SDU payload throughput tracker (demuxed DRB SDU length).
/// Call \c on_tick when a CRC-OK UL PDU is processed so idle windows can emit vol_bytes=0.
class ul_mac_shaped_thp_tracker
{
public:
  explicit ul_mac_shaped_thp_tracker(du_ue_index_t             ue_index_,
...
private:
  ...
  {
    // Use SCHED logger so lines match MAC-THP-DL tooling (grep / plot scripts).
    ...
    logger.info("UE{} [MAC-THP-UL] window_ms={:.0f} vol_bytes={} thp_kbps={:.3f}",
