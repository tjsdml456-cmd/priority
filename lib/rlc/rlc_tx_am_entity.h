...
struct rlc_tx_am_sdu_info {
  ...
  /// \brief PDB (ms) stamped at enqueue from the packet's DSCP profile.
...
private:
  ...
  /// \brief Drop SDUs whose HOL sojourn exceeds the current runtime PDB (queue + AM tx_window backlog).
