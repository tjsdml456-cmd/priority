...
static inline std::optional<uint8_t> gtpu_extract_dscp_from_ipv4(byte_buffer_view sdu)
{
  if (sdu.length() < 2) {
    return {};
  }
...
protected:
  ...
  {
    /* Log only when DSCP changes (DL from core) */
    std::optional<uint8_t> dscp = gtpu_extract_dscp_from_ipv4(byte_buffer_view(sdu_info.sdu));
    if (dscp.has_value()) {
      if (!last_dscp.has_value() || last_dscp.value() != dscp.value()) {
        logger.log_info("[GTPU] DL SDU DSCP changed to {} (IPv4 ToS) QFI={} sdu_len={} previous_dscp={}",
                        dscp.value(),
                        sdu_info.qos_flow_id,
                        sdu_info.sdu.length(),
                        last_dscp.has_value() ? static_cast<int>(last_dscp.value()) : -1);
...
private:
  ...
  std::optional<uint8_t>   last_dscp = {}; ///< Last DSCP seen (log only when it changes)
