...
void throughput_controller::set_config(const config& cfg_)
{
  ...
  cfg = cfg_;
  logger.info("Throughput controller config updated: target={}Mbps, kp={}, ki={}, kd={}, period={}ms",
...
void throughput_controller::set_target_throughput(du_ue_index_t ue_index, double target_mbps)
{
  ...
  logger.info("Target throughput set for UE{}: {}Mbps", static_cast<unsigned>(ue_index), target_mbps);
}
...
void throughput_controller::set_target_throughput_map(const std::unordered_map<du_ue_index_t, double>& ue_target_map)
{
  ...
  ue_target_throughput_map = ue_target_map;
  ...
  for (const auto& [ue_index, target_mbps] : ue_target_map) {
    ...
    if (it != ue_states.end()) {
      ...
      logger.info("Target throughput updated for UE{}: {}Mbps (from map)", static_cast<unsigned>(ue_index), target_mbps);
    } else {
      // UE doesn't exist yet, will be applied when UE is initialized
      logger.info("Target throughput pre-configured for UE{}: {}Mbps (will apply on initialization)", 
                  static_cast<unsigned>(ue_index), target_mbps);
    }
  }
  logger.info("Target throughput map applied for {} UEs", ue_target_map.size());
}
...
void throughput_controller::update_throughput(const scheduler_ue_metrics& metrics)
{
  ...
  if (it == ue_states.end()) {
    ...
    pid_state new_state;
    // Check if there's a pre-configured target for this UE, otherwise use default
    ...
    if (preconfigured_it != ue_target_throughput_map.end()) {
      ...
      logger.info("Using pre-configured target throughput for UE{}: {}Mbps", 
                  static_cast<unsigned>(metrics.ue_index), preconfigured_it->second);
    } else {
      new_state.target_mbps = cfg.target_throughput_mbps;
    }
    ...
    logger.info("Initialized throughput control for UE{}: target={}Mbps, initial_dscp={}",
                static_cast<unsigned>(metrics.ue_index), new_state.target_mbps, cfg.initial_dscp);
  }
  ...
  if (elapsed >= cfg.control_period_ms) {
    ...
    if (new_dscp != state.current_dscp) {
      ...
      logger.info("DSCP adjusted for UE{}: throughput={:.2f}Mbps (target={:.2f}Mbps), DSCP={}->{}",
...
void throughput_controller::enable_control(du_ue_index_t ue_index, bool enable)
{
  ...
  if (it != ue_states.end()) {
    ...
    logger.info("Throughput control {} for UE{}", enable ? "enabled" : "disabled", static_cast<unsigned>(ue_index));
  }
}
...
void throughput_controller::remove_ue(du_ue_index_t ue_index)
{
  ...
  logger.info("Removed UE{} from throughput control", static_cast<unsigned>(ue_index));
}
...
bool throughput_controller::is_control_enabled(du_ue_index_t ue_index) const
{
  std::lock_guard<std::mutex> lock(mutex);
  auto it = ue_states.find(ue_index);
  return it != ue_states.end() && it->second.enabled;
}
...
uint8_t throughput_controller::compute_dscp_adjustment(pid_state& state, double current_mbps, double target_mbps)
{
  ...
  // PID 출력을 priority 변화로 스케일링 (예: 1Mbps 오차 = priority 10 변화)
  const double priority_scale = 10.0; // PID 출력 1.0 = priority 10 변화
  ...
  int min_priority_diff = std::abs(static_cast<int>(current_priority) - target_priority);
  ...
  logger.debug("PID control (Priority-based): current={:.2f}Mbps, target={:.2f}Mbps, error={:.2f}, "
               "P={:.2f}, I={:.2f}, D={:.2f}, output={:.2f}, "
               "current_dscp={}, current_priority={}, target_priority={}, priority_change={}, best_dscp={}",
               current_mbps,
               target_mbps,
               state.error,
               p_term,
