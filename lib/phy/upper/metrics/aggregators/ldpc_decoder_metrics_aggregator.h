...
public:
  ...
  double get_avg_nof_iterations() const
  {
    return count.load(std::memory_order_relaxed)
               ? static_cast<double>(sum_nof_iterations.load(std::memory_order_relaxed)) /
  ...
  void reset()
  {
    ...
    sum_nof_iterations.store(0, std::memory_order_relaxed);
...
private:
  ...
  void on_new_metric(const ldpc_decoder_metrics& metrics) override
  {
    ...
    sum_nof_iterations.fetch_add(metrics.nof_iterations, std::memory_order_relaxed);
